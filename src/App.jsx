...
// Onboarding price tier based on how many trades the user picked.
// Maps 1 trade → Solo, 2 → Pro, 3+ → Elite. Keep these aligned with
// PLANS_SU monthlyPrice + PLANS price so the onboarding preview
// matches what's actually charged.
const getPrice = (n) => n >= 3 ? 249.99 : n === 2 ? 124.99 : 79.99;
...
function SignupScreen({ onComplete, onBack }) {
  ...
  // 'monthly' | 'yearly'. Yearly = 25% off the monthly × 12 (~3 months
  // free). Sticker shock is real, so we default to monthly — yearly is
  // the upsell visible behind a toggle.
  ...
  const PLANS_SU = [
    // Yearly = 25% off monthly × 12. Marketed as "3 months free" since
    // 25% off ≈ 3 of 12 months. Beats Jobber / JobNimbus / Housecall Pro
    // which all top out at ~20% off. Yearly amounts rounded to .99
    // endings; the marketed savings hold at 25% in copy.
    { id: 'solo', name: 'Solo',  monthlyPrice: 79.99,  yearlyPrice: 719.99,   trades: '1 trade',        desc: 'Just you — one trade, full power' },
    { id: 'pro',  name: 'Pro',   monthlyPrice: 124.99, yearlyPrice: 1124.99,  trades: 'Up to 3 trades', desc: 'Growing contractor, multiple trades — 1 tech seat included', popular: true },
    { id: 'all',  name: 'Elite', monthlyPrice: 249.99, yearlyPrice: 2249.99,  trades: 'All 56 trades',  desc: 'Full-service multi-trade contractor — every trade + 2 tech seats included' },
  ];
  ...
  // Formatter for the per-card price block. Shows either "$79.99/mo" or
  // "$719.99/yr · save $239.89" — keeps the yearly upsell legible.
      ...
      {step === 2 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* ── Monthly / Yearly toggle ──
              Annual prepay is 25% off — about 3 months free. Beats every
              competitor's 20% standard. The toggle swaps the price column
              on the cards below; the chosen plan slug ('pro' vs
              'pro_yearly') gets composed at submit time. */}
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
            <div style={{ display: 'inline-flex', background: C.raised, border: `1px solid ${C.border2}`, borderRadius: 999, padding: 3 }}>
              {[
                { id: 'monthly', label: 'Monthly' },
                { id: 'yearly',  label: 'Yearly · 3 months free' },
              ].map(opt => {
...
function Onboarding({ onComplete }) {
  ...
  const priceLabel = data.trades.length === 0 ? null
    : data.trades.length === 5 ? '$249.99/mo — all trades'
...
// ══════════════════════════════════════════════════════════════════════════════
const PLANS = [
  { name: 'Solo',  trades: 1,  price: 79.99,  desc: '1 trade — invoicing, quotes, payments',             badge: null         },
  { name: 'Pro',   trades: 3,  price: 124.99, desc: 'Up to 3 trades + 1 tech seat included',             badge: 'POPULAR'    },
  ...
  // bumped to $249.99 (2026 pricing refresh) to align with new market
  // positioning — undercuts ServiceTitan / JobNimbus enterprise tiers
  // while bundling the AI Scope Analyzer + 2 free tech seats.
  { name: 'Elite', trades: 56, price: 249.99, desc: 'All 56 trades + 2 tech seats included',             badge: 'BEST VALUE' },
];
...
function Billing({ user, setUser, payments }) {
  ...
  // instead of the misleading "Solo $79.99" fallback the default
